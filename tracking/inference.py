...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for position in legalPositions:
            manhattanDist = util.manhattanDistance(position, pacmanPosition)
            if emissionModel[manhattanDist] > 0:
                counter[position] = emissionModel[manhattanDist] * beliefs[position]
        if noisyDistance is None:
            jailPosition = self.getJailPosition()
            counter[jailPosition] = float(1)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        belief = util.Counter()
        legalPositions = self.legalPositions
        for exPos in legalPositions:
            newPostDist = self.getPositionDistribution(self.setGhostPosition(gameState, exPos))
            for key in newPostDist.keys():
                oldBeliefs = self.beliefs[exPos]
                belief[key] += oldBeliefs * newPostDist[key]
        belief.normalize()
        self.beliefs = belief
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [legal[i % len(legal)] for i in range(numParticles)]
        "*** END YOUR CODE HERE ***"
        ...
        if noisyDistance is None:
            ...
            for position in legal:
                ...
                if emissionModel[dist] > 0:
                    counter[position] = emissionModel[dist] * self.getBeliefDistribution()[position]
            if counter.totalCount():
                self.particles = [util.sample(counter) for i in range(self.numParticles)]                    
            else:
                self.initializeUniformly(gameState)            
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [util.sample(distr) for distr in
                          [self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)) for oldPos in self.particles]]       
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        product = list(itertools.product(self.legalPositions, repeat=numGhosts))
        length = len(product)
        partRange = range(self.numParticles)
        random.shuffle(partRange)
        self.particles = list(product[i % length] for i in partRange)  
        "*** END YOUR CODE HERE ***"
        ...
        for ghost in range(numGhosts):
            if noisyDistances[ghost] is None:
                ...
                for distribution in dist:
                    distance = util.manhattanDistance(distribution[ghost], pacmanPosition)
                    dist[distribution] *= emissionModels[ghost][distance]
        dist.normalize()
        if dist.totalCount():
            self.particles = [util.sample(dist) for i in range(numParticles)]
        else:
            self.initializeParticles()
            for ghost in range(numGhosts):
                if noisyDistances[ghost] is None:
                    self.particles = [self.getParticleWithGhostInJail(self.particles[i], ghost) for i in range(numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newPositionDistribution = getPositionDistributionForGhost(setGhostPositions(gameState,newParticle),i,self.ghostAgents[i])
